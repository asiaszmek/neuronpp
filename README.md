...
### Define Synapses
  ...
  * add single synapse:
   ```python
    ...
    cell.add_synapse(source=None, mod_name="Syn4P", seg=soma(0.5), netcon_weight=0.01, delay=1)
   ```
  ...
  * define NetStim (or VecStim) and pass it to synapses as a source while creating:
  ```python
    ...
    cell = Cell(name="cell")
    soma = cell.filter_secs(name="soma")
    cell.add_synapse(source=stim, seg=soma(0.5), mod_name="ExpSyn", netcon_weight=0.01, delay=1)
  ```
   ...
   * Make synaptic event (send external input to the synapse): 
   ...
   ```python
    cell = Cell(name="cell")
    soma = cell.filter_secs(name="soma")
    syns = cell.add_synapse(source=None, seg=soma(0.5), mod_name="ExpSyn", 
...
Debug any cell and synapse on interactive plot. 
  ...
  * Allows to easily plot synaptic weight (defined as MOD's RANGE variable) to see how the plasticity behaves in real time
  ```python
    ...
    syn = cell.add_synapse(source=None, mod_name="Exp2Syn", seg=soma, netcon_weight=0.1)
